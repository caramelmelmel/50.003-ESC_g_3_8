...
# This test allows us to check if the tenant emails that we expect to fail will actually fail the regex
...
# This test allows us to check if the tenant emails that we expect to fails will actually fails the regex
check = False
while (check == False):
    invalid_front = ["singhealth", "outlook"]
    valid = random.choice(invalid_front)
    output = fuzz(valid)
