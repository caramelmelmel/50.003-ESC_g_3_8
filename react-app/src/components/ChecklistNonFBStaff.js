...
class ChecklistFBStaff extends Component {
  state = {
    checklistNonFB: getAllChecklistItems(),
    score: 0,
    clickedItems: []
  };
        ...
        return <React.Fragment>
            <Container fluid>
                ...
                <tbody>
                    ...
                    {this.state.checklistNonFB.map(checklistItem => checklistItem.category == "professionalism" ?                
                    <tr>
                    ...
                    {this.state.checklistNonFB.map(checklistItem => checklistItem.category == "staff_hygiene" ?                
                    <tr>
                        <td className="checklist-body-style">{checklistItem.item}</td>
                        <td><input type="checkbox" aria-label="Checkbox for following text input" onClick={() => this.handleCheck(checklistItem.id)}/></td>
                        <td>
                    ...
                    {this.state.checklistNonFB.map(checklistItem => checklistItem.category == "environment_cleanliness" ?                
                    <tr>
                        <td className="checklist-body-style">{checklistItem.item}</td>
                        <td><input type="checkbox" aria-label="Checkbox for following text input" onClick={() => this.handleCheck(checklistItem.id)}/></td>
                        <td>
