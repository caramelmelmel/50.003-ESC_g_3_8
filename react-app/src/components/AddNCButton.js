...
class AddNCButton extends Component {
    ...
    handleAddNC = (itemId) => {
        ...
        //console.log("CLICKED ITEMS AT BUTTON: ", this.props.clickedItems);
    }


    render() { 
        return ( 
        <Link to={{pathname: `/add-nc-staff/${this.state.itemId}`, state: {itemId: this.state.itemId, clickedItems: this.props.clickedItems}}} onClick={(itemId) => this.handleAddNC(itemId)}>
