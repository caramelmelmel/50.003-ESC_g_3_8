...
class SKHTenant extends Component {
  constructor(props){
      super(props);
      this.state = {
  ...
  render() {
    ...
    const SKHtenants = tenants.filter((m) => m.institution.name === this.props.institutechosen.activeTab);
    

    const storename = []
    const currentaudit = []
    const pastaudit = []
    for (var i = 0; i < SKHtenants.length; i++) {
        pastaudit.push("null");
        currentaudit.push("null");
        storename.push(SKHtenants[i]);
        
        for (var j = 0; j < audits.length; j++) {

            if (SKHtenants[i].Tenant_id == audits[j].Tenant_id) {
                currentaudit[i] = audits[j].performancescore; 
            }
        }
        
        for (var z = 0; z < pastaudits.length; z++) {
            if (SKHtenants[i].Tenant_id === pastaudits[z].Tenant_id) {
