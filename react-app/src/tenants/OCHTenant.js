...
class OCHTenant extends Component {
  constructor(props){
      super(props);
      this.state = {
  ...
  render() {
    ...
    const OCHtenants = tenants.filter((m) => m.institution.name === this.props.institutechosen.activeTab);
    

    const storename = []
    const currentaudit = []
    const pastaudit = []
    for (var i = 0; i < OCHtenants.length; i++) {
        pastaudit.push("null");
        currentaudit.push("null");
        storename.push(OCHtenants[i]);
        
        for (var j = 0; j < audits.length; j++) {

            if (OCHtenants[i].Tenant_id == audits[j].Tenant_id) {
                currentaudit[i] = audits[j].performancescore; 
            }
        }
        
        for (var z = 0; z < pastaudits.length; z++) {
            if (OCHtenants[i].Tenant_id === pastaudits[z].Tenant_id) {
