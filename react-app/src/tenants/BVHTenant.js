...
class BVHTenant extends Component {
  constructor(props){
      super(props);
      this.state = {
  ...
  render() {
    ...
    const BVHtenants = tenants.filter((m) => m.institution.name === this.props.institutechosen.activeTab);
    

    const storename = []
    const currentaudit = []
    const pastaudit = []
    for (var i = 0; i < BVHtenants.length; i++) {
        pastaudit.push("null");
        currentaudit.push("null");
        storename.push(BVHtenants[i]);
        
        for (var j = 0; j < audits.length; j++) {

            if (BVHtenants[i].Tenant_id == audits[j].Tenant_id) {
                currentaudit[i] = audits[j].performancescore; 
            }
        }
        
        for (var z = 0; z < pastaudits.length; z++) {
            if (BVHtenants[i].Tenant_id === pastaudits[z].Tenant_id) {
